...
import { PessoaOcorrencia, Page, Pageable, ResultList } from '@/models';
...
export const initialState: State = {
  ...
  page: new Page<PessoaOcorrencia>(),
  pageable: new Pageable(),
  error: undefined,
};

export const getters: GetterTree<State, any> = {
  resultList: (state) => new ResultList<PessoaOcorrencia>(state.page, state.pageable, state.error),
...
export const mutations = {
  ...
  [Types.NO_RESULT](state: State) {
    state.loading = false;
    state.page = new Page<PessoaOcorrencia>();
...
export const actions = {
  async query(store: ActionContext<State, any>, { ocorrenciaId, pageable }) {
    try {
      ...
      const page = await OcorrenciasService.queryPessoa(ocorrenciaId, pageable).toPromise();
      if (page.isNotEmpty()) {
