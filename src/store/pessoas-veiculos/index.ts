...
import { FiltroPessoaVeiculo, Page, Pageable, ResultList, Veiculo } from '@/models';
...
export const initialState: State = {
  loading: false,
  filter: new FiltroPessoaVeiculo(),
  model: new Veiculo(),
...
export const mutations = {
  ...
  [Types.CLEAR](state: State) {
    state.loading = false;
    state.filter = new FiltroPessoaVeiculo();
    state.page = new Page<Veiculo>();
    state.pageable = new Pageable();
    state.error = undefined;
  },
...
export const actions = {
  async query(store: ActionContext<State, any>, { filter, pageable }) {
    try {
      ...
      const page = await VeiculosService.query(filter, pageable).toPromise();
      if (page.isNotEmpty()) {
