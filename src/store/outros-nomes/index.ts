...
import { OutroNome, Page, Pageable, ResultList } from '@/models';
import { OutrosNomesService } from '@/services';
...
export const initialState: State = {
  loading: false,
  pessoaId: undefined,
  page: new Page<OutroNome>(),
  pageable: new Pageable(),
  error: undefined,
};

export const getters: GetterTree<State, any> = {
  resultList: (state) => new ResultList<OutroNome>(state.page, state.pageable, state.error),
...
export const actions = {
  async query(store: ActionContext<State, any>, { pessoaId, pageable }) {
    try {
      ...
      const page = await OutrosNomesService.query(pessoaId, pageable).toPromise();
      if (page.isNotEmpty()) {
  ...
  async listOutrosNomes(store: ActionContext<State, any>, { pessoaId, filter, pageable }) {
    try {
      store.commit(Types.QUERY, { pessoaId, pageable });
      const page = await OutrosNomesService.listOutrosNomes(pessoaId, filter, pageable).toPromise();
      if (page.isNotEmpty()) {
        store.commit(Types.QUERY_SUCCESS, { page });
      } else {
        store.commit(Types.NO_RESULT);
