...
import { Alcunha, Page, Pageable, ResultList } from '@/models';
import { AlcunhasService } from '@/services';
...
export const initialState: State = {
  loading: false,
  pessoaId: undefined,
  page: new Page<Alcunha>(),
  pageable: new Pageable(),
  error: undefined,
};

export const getters: GetterTree<State, any> = {
  resultList: (state) => new ResultList<Alcunha>(state.page, state.pageable, state.error),
...
export const actions = {
  async query(store: ActionContext<State, any>, { pessoaId, pageable }) {
    try {
      ...
      const page = await AlcunhasService.query(pessoaId, pageable).toPromise();
      if (page.isNotEmpty()) {
  ...
  async listAlcunhas(store: ActionContext<State, any>, { pessoaId, filter, pageable }) {
    try {
      store.commit(Types.QUERY, { pessoaId, pageable });
      const page = await AlcunhasService.listAlcunhas(pessoaId, filter, pageable).toPromise();
      if (page.isNotEmpty()) {
        store.commit(Types.QUERY_SUCCESS, { page });
      } else {
        store.commit(Types.NO_RESULT);
