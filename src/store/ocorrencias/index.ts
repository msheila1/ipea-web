...
import { FiltroOcorrencia, Ocorrencia, Page, Pageable, ResultList } from '@/models';
...
export interface State {
  loading: boolean;
  filter: FiltroOcorrencia;
  page: Page<Ocorrencia>;
  pageable: Pageable;
  error?: Error;
}
...
export const initialState: State = {
  loading: false,
  filter: new FiltroOcorrencia(),
  page: new Page<Ocorrencia>(),
...
export const getters: GetterTree<State, any> = {
  ...
  resultList: (state) => new ResultList<Ocorrencia>(state.page, state.pageable, state.error),
};
...
export const mutations = {
  ...
  [Types.NO_RESULT](state: State) {
    state.loading = false;
    state.page = new Page<Ocorrencia>();
  ...
  [Types.CLEAR](state: State) {
    state.loading = false;
    state.filter = new FiltroOcorrencia();
    state.page = new Page<Ocorrencia>();
    state.pageable = new Pageable();
    state.error = undefined;
  },
...
export const actions = {
  async query(store: ActionContext<State, any>, { filter, pageable }) {
    try {
      ...
      const page = await OcorrenciasService.query(filter, pageable).toPromise();
      if (page.isNotEmpty()) {
