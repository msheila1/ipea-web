<template>
  <app-table-collapse header="Ocorrências relacionadas ao veículo" :loading="loading" :result="result" @page-change="onPageChange" @refresh="onRefresh">
    <table class="table table-striped table-hover">
      ...
      <tbody>
        <tr v-for="(ocorrencia, index) in result.page.data" :key="ocorrencia.id">
          ...
          <td>
            <span v-if="ocorrencia.confidencial">{{ ocorrencia.delegacia }}</span>
            <router-link v-else :to="toOcorrenciaDetail(ocorrencia)" class="text-dark">
          ...
          <td class="text-center">
            <el-dropdown v-if="!ocorrencia.confidencial" @command="onActions" trigger="click">
              <span class="el-dropdown-link">
                <i id="action-veiculo-ocorrencia-table"  class="icon-menu9"></i>
              </span>
