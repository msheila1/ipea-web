    ...
    <table class="table table-striped table-hover table-sm">
      ...
      <tbody>
        <tr v-for="(veiculo, index) in result.page.data" :key="veiculo.id">
          ...
          <td>
            <router-link :to="toVeiculoDetail(veiculo)" class="text-dark">{{ veiculo.envolvimento }}</router-link>
          </td>
          <td>
            <router-link :to="toVeiculoDetail(veiculo)" class="text-dark">{{ veiculo.natureza }}</router-link>
          </td>
          <td class="text-center">
            <el-dropdown @command="onActions" trigger="click">
              <span class="el-dropdown-link">
                <i id="action-ocorrencia-veiculo-table" class="icon-menu9"></i>
              </span>
...
import { Pageable, ResultList, VeiculoOcorrencia } from '@/models';

@Component
export default class VeiculoTable extends Vue {
  @Prop({ default: () => new ResultList<VeiculoOcorrencia>() }) private result!: ResultList<VeiculoOcorrencia>;
  @Prop() private loading!: boolean;

  private onActions(command: any): void {
    if ('detail' === command.event) {
      this.onDetail(command.data as VeiculoOcorrencia);
    }
  }
  ...
  private onDetail(veiculo: VeiculoOcorrencia): void {
    this.$emit('detail', veiculo);
  }

  private toVeiculoDetail(veiculo: VeiculoOcorrencia): Location {
    return { name: 'VeiculoDetail', params: { id: veiculo.id }};
  }
}
</script>
