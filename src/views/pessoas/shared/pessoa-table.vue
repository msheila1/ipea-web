          ...
          <td class="text-center">
            <el-dropdown @command="onActions" trigger="click">
              <span class="el-dropdown-link">
                <i id="action-pessoa-table" class="icon-menu9"></i>
              </span>
...
export default class PessoaTable extends Vue {
  ...
  // TODO Pode virar um componente (avatar)
  private css(): string {
    const array =
      ['bg-teal-400',
      'bg-blue',
      ...
      'bg-success-400',
      'bg-danger-400'];
    return array[_.random(0, 11)];
  }

  private firstChar(nome: string): string {
    return _.upperFirst(nome).charAt(0);
  }
}
</script>
